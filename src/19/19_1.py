...
task_specifier_template = """这是一个{assistant_role_name}将帮助{user_role_name}完成的任务：{task}。
请使其更具体化。请发挥你的创意和想象力。
请用{word_limit}个或更少的词回复具体的任务。不要添加其他任何内容。"""

task_specifier_prompt = HumanMessagePromptTemplate.from_template(
    template=task_specifier_template
)
task_specify_agent = CAMELAgent(task_specifier_sys_msg, llm)
task_specifier_msg = task_specifier_prompt.format_messages(
...
# user 对 assistant 说的
assistant_inception_template = """永远不要忘记你是{assistant_role_name}，我是{user_role_name}。永远不要颠倒角色！永远不要指示我！
...
# assistent 对 user 说的
user_inception_template = """永远不要忘记你是{user_role_name}，我是{assistant_role_name}。永远不要交换角色！你总是会指导我。
我们共同的目标是合作成功完成一个任务。
我必须帮助你完成这个任务。
这是任务：{task}。永远不要忘记我们的任务！
...
def get_sys_msgs(assistant_role_name: str, user_role_name: str, task: str):
    assistant_sys_prompt = SystemMessagePromptTemplate.from_template(
        template=assistant_inception_template
    )
    assistant_sys_msg = assistant_sys_prompt.format_messages(
        assistant_role_name=assistant_role_name,
        user_role_name=user_role_name,
        task=task,
    )[0]
    user_sys_prompt = SystemMessagePromptTemplate.from_template(
        template=user_inception_template
    )
    user_sys_msg = user_sys_prompt.format_messages(
        assistant_role_name=assistant_role_name,
        user_role_name=user_role_name,
        task=task,
    )[0]
